...
def Ax_func(a0, Aa, tod, tod_mask, pix, nside, bl):
    ...
    m = maps.Maps(nside)
    # m = (A^T A)^-1 A^T d
    m.tod2map(d.reshape(-1), None, pix.reshape(-1))
    # d1 = A m
    d1 = m.map2tod(pix)
...
def b_func(x0, tod, tod_mask, pix, nside, bl):
    """Return b = F^T Z d, where Z = I - A (A^T A)^-1 A^T."""
    nt, nb = tod.shape
    na  = np.int(np.ceil(tod.shape[0] / np.float(bl)))
...
def maker(tod, tod_mask, pix, nside, bl, max_iter=200, tol=1.0e-6, verbose=False):
    if len(tod.shape) == 1:
        tod = tod.reshape(-1, 1)
        if tod_mask is not None:
    ...
    a = cg.cg_solver(a0.reshape(-1), Ax_func, b_func, args=(tod, tod_mask, pix, nside, bl), max_iter=max_iter, tol=tol, verbose=verbose)
    ...
    if tod_mask is not None:
        tod_mask = tod_mask.reshape(-1)
    m.tod2map((tod - Fa).reshape(-1), tod_mask, pix.reshape(-1))
